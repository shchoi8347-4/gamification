...
public class GameServiceImplTest {
  ...
  public void processCorrectAttemptForScoreBadgeTest() {
    ...
    // when
    GameStats iteration = gameService.newAttemptForUser(userId, attemptId, true); // 정답
  ...
  public void processCorrectAttemptForLuckyNumberBadgeTest() {
    ...
    // 행운의 숫자가 포함된 답안
    MultiplicationResultAttempt attempt = new MultiplicationResultAttempt(
            "john_doe", 42, 10, 420, true);  // 42가 행운의 숫자
  ...
  public void retrieveStatsForUserTest() {
    ...
    given(scoreCardRepository.getTotalScoreForUser(userId))
            .willReturn(totalScore);
    given(badgeCardRepository.findByUserIdOrderByBadgeTimestampDesc(userId))
            .willReturn(Collections.singletonList(badgeCard)); // 실버 배지 카드를 무조건 리턴하도록 함

    // when
    GameStats stats = gameService.retrieveStatsForUser(userId); // 실버 배지 카드를 담은  GameStats 객체가 리턴됨

    // assert - 점수 카드 하나와 첫 번째 정답 배지를 획득
    assertThat(stats.getScore()).isEqualTo(totalScore);
    assertThat(stats.getBadges()).containsOnly(Badge.SILVER_MULTIPLICATOR); // 리턴된 GameStats 객체에 실버 배지 카드가 있는지 확인함
